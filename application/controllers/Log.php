...
class Log extends Base_Controller {
  ...
  public function index()
  {
    if ($this->method == 'GET')
    {
      ...
      $params = array();
      $params['sort'][] = array(
        'sort_by'           => ( ! empty($request_data['sort_by']) ? $request_data['sort_by'] : 'submit'),
      ...
      if ( ! empty($request_data['method']))
        $params['method'] = $request_data['method'];

      if ( ! empty($request_data['url']))
        $params['url'] = 'LIKE::%' . $request_data['url'] . '%';
      // END: Preparing request parameters

      // GET DATA
      $get_data = $this->log_model->read($params);
